...
/**
 * 仿照原版 WaveInfoDialog 的魔法波次编辑器。
 * 额外增加“神通”选择按钮，可以给魔法单位附加 ShenTong。
 */
public class MagicWaveEditor extends BaseDialog {
    ...
    public MagicWaveEditor() {
        ...
        buttons.button(Core.bundle.get("waves.edit"), Icon.edit, () -> {
            BaseDialog dialog = new BaseDialog(Core.bundle.get("waves.edit"));
            ...
            dialog.cont.table(Tex.button, t -> {
                ...
                t.button(Core.bundle.get("waves.copy"), Icon.copy, style, () -> {
                    ui.showInfoFade(Core.bundle.get("waves.copied"));
                    Core.app.setClipboardText(maps.writeWaves(groups));
                    dialog.hide();
                }).disabled(b -> groups == null || groups.isEmpty()).marginLeft(12f).row();

                t.button(Core.bundle.get("waves.load"), Icon.download, style, () -> {
                    try {
                        Seq<SpawnGroup> loaded = maps.readWaves(Core.app.getClipboardText());
                        groups = convertAll(loaded);
                        buildGroups();
                    } catch (Exception e) {
                        Log.err(e);
                    ...
                    dialog.hide();
                }).disabled(Core.app.getClipboardText() == null || !Core.app.getClipboardText().startsWith("["))
                        .marginLeft(12f).row();
                ...
                t.button("@settings.reset", Icon.refresh, style,
                        () -> ui.showConfirm("@confirm", "@settings.clear.confirm", () -> {
                            groups = convertAll(JsonIO.copy(waves.get()));
                            buildGroups();
                            dialog.hide();
                        })).marginLeft(12f);
            });
        ...
        buttons.button(Core.bundle.get("waves.random"), Icon.refresh, () -> {
            groups = convertAll(Waves.generate(1f / 10f));
            buildGroups();
        }).width(200f);
    }
    ...
    void setup() {
        groups = convertAll(
                state.rules.spawns == null || state.rules.spawns.isEmpty() ? waves.get() : state.rules.spawns);
        if (groups == null)
            groups = new Seq<>();
        ...
        cont.stack(new Table(Tex.clear, main -> {
            main.table(s -> {
                ...
                s.button(Icon.units, Styles.emptyi, () -> showUnits(type -> filterType = type, true)).size(46f)
                        .tooltip("@waves.filter")
                        .update(b -> b.getStyle().imageUp = filterType != null
                                ? new TextureRegionDrawable(filterType.uiIcon)
                                : Icon.filter);
            }).growX().pad(6f).row();
            ...
            main.table(t -> {
                t.button("@add", () -> {
                    showUnits(type -> groups.add(new MagicSpawnGroup(type)), false);
                    buildGroups();
                }).growX().height(70f);
        ...
        }).width(panelWidth()).growY();

        cont.add(graph).grow().minWidth(200f);
        ...
        if (groups != null) {
            ...
            for (SpawnGroup group : groups) {
                if (group.effect == StatusEffects.none)
                    group.effect = null;
                if ((search >= 0 && group.getSpawned(search) <= 0) || (filterType != null && group.type != filterType))
                    continue;
                ...
                table.table(Tex.button, t -> {
                    ...
                    t.button(b -> {
                        ...
                        b.label(() -> (group.begin + 1) + "").color(Color.lightGray).minWidth(45f)
                                .labelAlign(Align.left).left();

                        b.button(Icon.copySmall, Styles.emptyi, () -> {
                            groups.insert(groups.indexOf(group) + 1, expandedGroup = copyToMagic(group).copy());
                            buildGroups();
                        }).pad(-4).size(40f).tooltip("@editor.copy");

                        b.button(
                                group.effect != null ? new TextureRegionDrawable(group.effect.uiIcon) : Icon.logicSmall,
                                Styles.emptyi, () -> showEffects(group)).pad(-4).size(40f).scaling(Scaling.fit)
                                .tooltip(group.effect != null ? group.effect.localizedName : "@none");

                        ImageButton stBtn = b
                                .button(Icon.menu, Styles.emptyi, () -> showShenTongs((MagicSpawnGroup) group))
                                .pad(-4).size(40f).tooltip(shenTongName(group)).get();
                        stBtn.update(() -> {
                            TextureRegionDrawable icon = null;
                            if (group instanceof MagicSpawnGroup mg && mg.shenTongId >= 0) {
                                ShenTong st = ShenTong.shengTongMap.get(mg.shenTongId);
                                if (st != null && st.icon != null)
                                    icon = st.icon;
                            }
                            stBtn.getStyle().imageUp = icon != null ? icon : Icon.logicSmall;
                        });

                        b.button(Icon.unitsSmall, Styles.emptyi, () -> showUnits(type -> group.type = type, false))
                                .pad(-4).size(40f).tooltip("@stat.unittype");
                        ...
                        b.button(Icon.cancel, Styles.emptyi, () -> {
                            ...
                            t.remove();
                            buildGroups();
                        }).pad(-4).size(40f).padRight(-12f).tooltip("@waves.remove");
                    ...
                    if (expandedGroup == group) {
                        t.table(spawns -> {
                            ...
                            spawns.field(group.end == never ? "" : (group.end + 1) + "", TextFieldFilter.digitsOnly,
                                    text -> {
                                        if (Strings.canParsePositiveInt(text)) {
                        ...
                        t.table(a -> {
                            ...
                            a.add(" + ");
                            a.field(Strings.fixed(
                                    Math.max((Mathf.zero(group.unitScaling) ? 0 : 1f / group.unitScaling), 0), 2),
                                    TextFieldFilter.floatsOnly, text -> {
                        ...
                        t.table(a -> {
                            ...
                            a.button(
                                    b -> b.image(Tex.whiteui).size(iconSmall).update(
                                            i -> i.setColor(group.team == null ? Color.clear : group.team.color)),
                                    Styles.squarei,
                                    () -> MapObjectivesDialog.showTeamSelect(true, team -> group.team = team))
                                    .size(38f);
                        }).padTop(0).row();
                            ...
                            a.button("", () -> {
                                ...
                                dialog.cont.pane(p -> {
                                    ...
                                    if (spawner.getSpawns().size >= max) {
                                        p.add(Core.bundle.format("waves.spawn.first", max)).colspan(cols).padBottom(4)
                                                .row();
                                    }
                                ...
                                dialog.addCloseButton();
                                dialog.show();
                            }).width(160f).height(36f).get().getLabel()
                                    .setText(() -> group.spawn == -1 ? "@waves.spawn.all"
                                            : Point2.x(group.spawn) + ", " + Point2.y(group.spawn));

                        }).padBottom(8f).row();
                    }
                }).width(panelWidth()).pad(8);
    ...
    void showEffects(SpawnGroup group) {
        ...
        dialog.cont.pane(p -> {
            ...
            for (StatusEffect effect : content.statusEffects()) {
                if (effect.isHidden() || effect.reactive)
                    continue;
                p.button(t -> {
                    t.left();
    ...
    void showShenTongs(MagicSpawnGroup group) {
        BaseDialog dialog = new BaseDialog(Core.bundle.get("waves.shentong.select", "选择神通"));
        dialog.cont.pane(p -> {
            p.defaults().pad(2).fillX();
            p.button(t -> {
                t.left();
            ...
            int i = 1;
            for (ShenTong st : ShenTong.shengTongMap.values()) {
                p.button(t -> {
                    t.left();
                    t.image(st.icon != null ? st.icon : Icon.add).size(8 * 4).scaling(Scaling.fit).padRight(2f);
                    t.add(st.name());
                }, () -> {
                    group.shenTongId = st.id;
                    dialog.hide();
                    buildGroups();
                }).margin(12f);
                if (++i % 3 == 0)
    ...
    String shenTongName(SpawnGroup group) {
        if (group instanceof MagicSpawnGroup mg && mg.shenTongId >= 0) {
            ShenTong st = ShenTong.shengTongMap.get(mg.shenTongId);
            return st != null ? st.name() : "@none";
        }
        return "@none";
    }

    Seq<SpawnGroup> convertAll(Seq<SpawnGroup> src) {
        Seq<SpawnGroup> out = new Seq<>();
        if (src != null) {
            for (SpawnGroup g : src) {
                out.add(copyToMagic(g));
            }
        }
        return out;
    }

    MagicSpawnGroup copyToMagic(SpawnGroup g) {
        if (g instanceof MagicSpawnGroup mg) {
            return mg.copy();
        }

        MagicSpawnGroup m = new MagicSpawnGroup(g.type);
        m.begin = g.begin;
        m.end = g.end;
        m.spacing = g.spacing;
        m.max = g.max;
        m.unitScaling = g.unitScaling;
        m.shields = g.shields;
        m.shieldScaling = g.shieldScaling;
        m.unitAmount = g.unitAmount;
        m.spawn = g.spawn;
        m.payloads = g.payloads;
        m.effect = g.effect;
        m.items = g.items;
        m.team = g.team;
        m.shenTongId = -1;
    ...
    /** 左侧配置面板宽度：小屏占一半，大屏最大 420 */
    float panelWidth() {
        return Math.min(Core.graphics.getWidth() / 2f, 420f);
    }
}
